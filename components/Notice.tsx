...
const Notice = (props: { bulletin: CoinOrderBulletin[] }) => {
    ...
    return (<div
        ...
        {loading&&<NoticeBar background={'#fff'} rightIcon={<Eye fill={'#1EA68A'} className={'ml-1'}/>} className={'flex-1'} scrollable text={ <div className={'h-full flex items-center [&>div]:mr-3 [&>div]:pr-3 last:!border-none'}>
                {bulletin.map((it, index) => (
                    <div key={index} className={'!text-[#333340] items-center label-4-regular flex [&>span]:whitespace-nowrap border-r-[1px] border-solid border-[rgba(58,58,89,0.33)]'}>
